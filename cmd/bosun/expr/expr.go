...
func (e *Expr) Execute(c opentsdb.Context, g graphite.Context, l LogstashElasticHosts, cache *cache.Cache,
	T miniprofiler.Timer, now time.Time, autods int, unjoinedOk bool, search *search.Search,
	squelched func(tags opentsdb.TagSet) bool, history AlertStatusProvider, metadata MetadataProvder) (r *Results, queries []opentsdb.Request, err error) {
	if squelched == nil {
		squelched = func(tags opentsdb.TagSet) bool {
			return false
		}
